...
- **指令**：`/showcase` 指令组（20 个子指令，覆盖配置读取、规则解析、KV、i18n、平台信息、LLM 调用、
  T2I 卡片、消息链、多轮会话、主动推送、工具管理），以及 `permission_type` / `platform_adapter_type` /
  `regex` / `custom_filter` 四个顶层过滤器示例；保留模板原指令 `/helloworld`
...
- **LLM**：`@filter.llm_tool` 函数工具（`showcase_echo`）、类式 `FunctionTool`（`showcase_status`）+
  `context.add_llm_tools()`、`context.llm_generate` 直调模型、`context.tool_loop_agent` 工具循环、
...
- **其它小 API**：`event.react()`、`event.send_typing()/stop_typing()`、`event.should_call_llm(False)`、
  平台原生调用 `event.bot.api.call_action()`（`/showcase-recall`，仅 QQ/aiocqhttp）
...
- **工程结构**：`showcase/` 子包承载三块有状态逻辑（推送 / 向导 / 类式工具），装饰器全部留在 `main.py`
- **文档**：README（含 core 与 WebUI 两条版本线的能力对照表、进阶扩展点说明）、本更新日志
